...
class ModelExtensionPaymentNovalnet extends model
{
    ...
    /**
     * Fraud module validation process
     *
     * @param  $payment_name
     * @return string
     */
    public function validateCallbackFields($payment_name)
    {
        if(!empty($this->session->data[$payment_name]['fraud_module_enabled']) && $this->config->get($payment_name . '_fraud_enable') != 'none') {
            $fraud_module = $this->config->get($payment_name . '_fraud_enable');
            $callback_field = trim($this->request->request[$payment_name . '_pinby_' . $fraud_module]);
            if(empty($callback_field) || !preg_match('/^[0-9+]+$/', $callback_field)) {
                return $this->language->get('error_' . $fraud_module . '_empty');
            }
        }
        return '';
    }
    ...
    /**
     * Perform fraud module process
     *
     * @param  $response
     * @param  $payment_name
     * @return array
     */
    public function pinbyCallbackResponseProcess($response, $payment_name) {
        $json = array();
            $this->session->data[$payment_name]['tid'] = $response['tid'];
            $this->session->data[$payment_name]['test_mode'] = $response['test_mode'];
            $this->session->data[$payment_name]['order_no'] = $response['order_no'];
            $this->session->data[$payment_name]['amount'] = $this->amount;
            if ($payment_name == 'novalnet_invoice') {
                $this->session->data[$payment_name]['invoice_iban'] = $response['invoice_iban'];
                $this->session->data[$payment_name]['invoice_bic'] = $response['invoice_bic'];
                $this->session->data[$payment_name]['invoice_bankname'] = $response['invoice_bankname'];
                $this->session->data[$payment_name]['invoice_bankplace'] = $response['invoice_bankplace'];
                $this->session->data[$payment_name]['due_date'] = $response['due_date'];
            }else if ($payment_name == 'novalnet_sepa') {
                $this->session->data[$payment_name]['bankaccount_holder'] = $response['bankaccount_holder'];
                $this->session->data[$payment_name]['iban'] = $response['iban'];
            }
            $json_value = $this->config->get($payment_name . '_fraud_enable') ? 'display_callback_info' : '';
            $json[$json_value]                                  = ($this->config->get($payment_name . '_fraud_enable') == 'sms') ? $this->language->get('novalnet_fraud_sms_msg') : $this->language->get('novalnet_fraud_tel_msg');
        return $json;
    }
    ...
    public function getMerchantDetails() {
        ...
        if (!empty($_SESSION['nn']['nn_aff_id'])) {
            $query = $this->db->query('SELECT affiliate_configuration FROM ' . DB_PREFIX . 'novalnet_affiliate_account WHERE aff_id=' . $_SESSION['nn']['nn_aff_id'] . ' ORDER BY id desc');

            if (!empty($query->row['affiliate_configuration'])) {
                $affiliate_details = (array) json_decode($query->row['affiliate_configuration']);
                $vendor_parameters['vendor'] = $_SESSION['nn']['nn_aff_id'];
                $vendor_parameters['auth_code'] = $affiliate_details['aff_authcode'];
                $this->session->data['nn']['payment_access_key'] = $affiliate_details['aff_accesskey'];
            }
        } else if(!empty($this->session->data['customer_id'])) {
            $query = $this->db->query('SELECT ad.aff_id, ad.affiliate_configuration FROM ' . DB_PREFIX . 'novalnet_affiliate_account ad INNER JOIN ' . DB_PREFIX . 'novalnet_affiliate_user ud ON ad.aff_id = ud.aff_id WHERE ud.customer_id =' . $this->session->data['customer_id'] . ' ORDER BY ud.id DESC LIMIT 1');
            if (!empty($query->row['affiliate_configuration'])) {
                $affiliate_details = (array) json_decode($query->row['affiliate_configuration']);
                $vendor_parameters['vendor'] = $_SESSION['nn']['nn_aff_id'] = $query->row['aff_id'];
                $vendor_parameters['auth_code'] = $affiliate_details['aff_authcode'];
                $this->session->data['nn']['payment_access_key'] = $affiliate_details['aff_accesskey'];
            }
    ...
    public function getParameters($payment_name, $order_info = '') {
        ...
        $system_parameters = array(
            ...
            'system_version' => VERSION . '-' . 'NN_11.3.1',
        ...
        $additional_parameters = array_filter(array(
            ...
            'referrer_id' => $this->config->get('payment_novalnet_referrer_id'),
    ...
    public function handleSubscriptionProcess($response, $order_info, $transaction_comments) {
        ...
        foreach($recurring_products as $recurring_product ) {
            ...
            $recurring_id = $this->model_checkout_recurring->addRecurring($order_info['order_id'], $recurring_description, $recurring_product['recurring']);
    ...
    public function transactionSuccess($response, $payment_name) {
        ...
        $this->insertAffiliateDetails($order_info, $configuration_details['vendor']);
     ...
     * @return  none
     */
    public function insertAffiliateDetails($order_info, $vendor_id)
    {
        if(!empty($_SESSION['nn']['nn_aff_id'])) {
            $this->dbInsert('novalnet_affiliate_user', array(
                'aff_id',
                'customer_id',
                'aff_order_no'
            ), array(
    ...
    public function unsetSessionValues($payment_name)
    {
        ...
        if (!empty($this->session->data['nn']['nn_aff_id'])) {
            unset($this->session->data['nn']['nn_aff_id']);
        }
    ...
    {
        $novalnet_comments  = '';
        $novalnet_comments .= $this->prepareBankDetailsComments($response, $order_info, $payment_name);
     ...
     * @return string
     */
    public function prepareBankDetailsComments($response, $order_info)
    {
        $due_date = isset($response['due_date']) ? $response['due_date'] : '0000-00-00';
        $novalnet_comments = '<br><br>' . $this->language->get('text_novalnet_bank_comments') . '<br>';
        $novalnet_comments .= $this->language->get('text_novalnet_due_date') . date($this->language->get('date_format_short'), strtotime($due_date)) . '<br>';
        ...
        $novalnet_comments .= $this->language->get('text_novalnet_amount') . $this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value']) . '<br><br>';
        ...
        $novalnet_comments .= $this->language->get('text_payment_reference') . ' 2: BNR-' . trim($this->config->get('payment_novalnet_project_id')) . '-' . $response['order_no'] . '<br>';
        return $novalnet_comments;
    }
    ...
    public function performPaymentCall($parameters, $url = 'https://payport.novalnet.de/paygate.jsp', $payment_call = TRUE) {
        ...
        $proxy = trim($this->config->get('novalnet_proxy'));
        $ch    = curl_init($url);
    ...
    /**
     * Validate the fraud module pin
     *
     * @return      array
     */
    public function pinValidation() {

        if (empty($this->request->request['new_pin'])) {
            $pin = trim($this->request->request['pin_data']);

            // if the pin is empty, then display error message
            if (empty($pin)) {
                return $this->language->get('error_pin_empty');
            }

            // if the pin is not alpha numeric value , then display error message
            if (!$this->isAlphaNumeric($pin)) {
                return $this->language->get('error_pin_not_valid');
            }
        }
    }

    /**
     * Check whether the value is alpha numeric or not
     *
     * @param       $input
     * @return      boolean
     */
    ...
    {
        $input = trim($input);
        return ($input != '' && preg_match("/^[0-9a-zA-Z]+$/", $input)); // check if the given pin value is alpha numeric or not
    }
     ...
     * @return      array
     */
    public function generateXmlRequest($request_param)
    {
        ...
        // perform second call process
        return (array)simplexml_load_string($this->performPaymentCall($urlparam, $this->getUrl('nn_infoport'), FALSE));
    }
     ...
     *
     */
    public function getFraudModuleDetails($payment_name, $order_info, $data)
    {
        $data['enabled_fraud_module'] = $this->config->get($payment_name . '_fraud_enable');
        $this->session->data[$payment_name]['fraud_module_enabled'] = false;
        $data['show_fraud_module'] = false;
        if ($data['enabled_fraud_module'] != 'none') {
            $minimum_amount = (int)trim($this->config->get($payment_name . '_fraud_min_val')) / 100;
            $order_amount   = (int)$this->session->data['order_id'];

            if ($this->validateAllowedCountries($order_info['payment_iso_code_2']) && (empty($minimum_amount) || ($order_amount >= $minimum_amount))) {

                $data['fraud_check_value'] = $order_info['telephone'];
                $data['fraud_check_label'] = $this->language->get('text_fraud_tel');
                if ($data['enabled_fraud_module'] == 'sms') {
                    $data['fraud_check_label'] = $this->language->get('text_fraud_mobile');
                }
                $data['show_fraud_module'] = true;
                $this->session->data[$payment_name]['fraud_module_enabled'] = $data['enabled_fraud_module'];
            }
        }
     ...
     * @return array
     */
    public function formFraudModuleParams($parameters, $payment_name) {

        if(!empty($this->session->data[$payment_name]['fraud_module_enabled']) && in_array($parameters['key'], array('27', '37'))) {
            if ($this->config->get($payment_name . '_fraud_enable') == 'callback') {
                $parameters['tel']             = $this->request->request[$payment_name . '_pinby_callback'];
                $parameters['pin_by_callback'] = '1';
            } elseif ($this->config->get($payment_name . '_fraud_enable') == 'sms') {
                $parameters['mobile']     = $this->request->request[$payment_name . '_pinby_sms'];
    ...
    /**
     * Send the Transmit PIN again request
     *
     * @param   $payment_name
     * @return  array
     */
    public function sendPinConfirmation($payment_name)
    {
        // Validate fraud modules input fields for second call and new pin generation
        $config = $this->getMerchantDetails();
        $request_parameters = array(
            'vendor_id'       => $config['vendor'],
            'vendor_authcode' => $config['auth_code'],
            'tid'             => $this->session->data[$payment_name]['tid'],
            'remote_ip'       => $this->getIpAddress('REMOTE_ADDR'),
        );

        if(!empty($this->request->request['new_pin'])) {
            $request_parameters['request_type'] = 'TRANSMIT_PIN_AGAIN';
        } else {
            $request_parameters['request_type'] = 'PIN_STATUS';
            $request_parameters['pin'] = trim($this->request->request['pin_data']);
        }

        // Sending the transaction PIN to server.
        $xml_response = $this->generateXmlRequest($request_parameters); // generate second call parameters as a xml format

        $message      = $this->setResponseMessage($xml_response);

        $this->session->data[$payment_name]['status']      = !empty($xml_response['status']) ? $xml_response['status'] : '';
        $this->session->data[$payment_name]['status_text'] = $message;

        if (!empty($xml_response['status']) && $xml_response['status'] == 100) {

            // Get the new TID status.
            $this->session->data[$payment_name]['tid_status'] = $xml_response['tid_status'];

        } else if (!empty($xml_response['status']) && $xml_response['status'] == '0529006') {

            // Hide payment notification.
            $this->session->data[$payment_name . '_max_time'] = time() + (30 * 60);
            $this->session->data[$payment_name . '_status_text'] = $message;
            $this->unsetSessionValues($payment_name);
            return $xml_response;
        }
        return (!empty($this->session->data[$payment_name]) ? $this->session->data[$payment_name] : '');
    }
    ...
    public function setResponseMessage($data)
    {
        ...
        } elseif(isset($data['pin_status'])) {
            return (string)$data['pin_status']->status_message;
        } else {
    ...
    public function showPaymentMethod($payment_name, $address, $total) {
        ...
        $status = ($minimum_total > 0 && $minimum_total > $total) ? false : ((!$geo_zone_id) ? true : (($query->num_rows) ? true : false ));
        if ($status) {
            return array(
                'code' => $payment_name,
