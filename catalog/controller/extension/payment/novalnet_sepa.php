...
class ControllerExtensionPaymentNovalnetSepa extends Controller
{
    ...
    private $payment_method = 'novalnet_sepa';
    ...
    public function index()
    {
        ...
        $data = $this->model_extension_payment_novalnet->getFraudModuleDetails('payment_novalnet_sepa', $order_info, $data);
    ...
    {   
        ...
        $this->load->model('checkout/order');
        $this->load->model('extension/payment/novalnet');
        if (empty($this->session->data['payment_novalnet_sepa']['tid'])) {
            ...
            $this->json['error'] = empty($this->json['error']) ? $this->model_extension_payment_novalnet->validateCallbackFields('payment_novalnet_sepa') : $this->json['error'];
            ...
            if(empty($parameters['payment_ref'])) {                             
                ...
                $parameters = $this->model_extension_payment_novalnet->formFraudModuleParams($parameters, 'payment_novalnet_sepa');
            } else { 
        ...
        // Perform payment call.
            ...
            if(!empty($server_response['status']) && $server_response['status'] == '100') {
                if(!empty($parameters['pin_by_callback']) || !empty($parameters['pin_by_sms'])) {
                    $this->session->data['payment_novalnet_sepa'] = $server_response;
                    $this->json = $this->model_extension_payment_novalnet->pinbyCallbackResponseProcess($server_response, 'payment_novalnet_sepa');
                    $this->response->addHeader('Content-Type: application/json');
                    $this->response->setOutput(json_encode($this->json));
                    return false;
                }
            ...
            $server_response = $this->model_extension_payment_novalnet->sendPinConfirmation('payment_novalnet_sepa');
        }
    ...
    public function sepaDetails($data, $order_info) {
        ...
        // Check for PIN call.
        $data['process_pin_call']     = !empty($this->session->data['novalnet_sepa']['tid']);
