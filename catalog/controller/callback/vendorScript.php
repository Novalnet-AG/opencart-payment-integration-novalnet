...
class ControllerCallbackVendorScript extends Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($this->capture_params['vendor_activation']) && $this->capture_params['vendor_activation'] == '1') {

            $this->updateAffiliateDetail(array(
                'vendor_id'         => $this->capture_params['vendor_id'],
                'vendor_authcode'   => $this->capture_params['vendor_authcode'],
                'product_id'        => $this->capture_params['product_id'],
                'product_url'       => !empty($this->capture_params['product_url']) ? $this->capture_params['product_url'] : '',
                'activation_date'   => !empty($this->capture_params['activation_date']) ? $this->capture_params['activation_date'] : date('Y-m-d H:i:s'),
                'aff_id'            => $this->capture_params['aff_id'],
                'aff_accesskey'     => $this->capture_params['aff_accesskey'],
                'aff_authcode'      => $this->capture_params['aff_authcode']
            ));
    ...
    public function secondLevelProcess()
    {
        ...
        $this->displayMessage('Novalnet callback received. Callback Script executed already. Refer Order :' . (!empty($this->transaction_history['order_no']) ? $this->transaction_history['order_no'] : ''));
    }
        ...
        if(in_array($this->capture_params['payment_type'], array('INVOICE_START', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'CREDITCARD', 'PAYPAL')) && in_array($this->transaction_history['gateway_status'], array(75, 91, 99, 98, 85)) && in_array($this->capture_params['tid_status'], array(91, 99, 100))) {
            ...
            if($this->capture_params['tid_status'] == 100 && $this->capture_params['status'] == 100 && in_array($this->transaction_history['gateway_status'], array(91, 99, 98, 85))) {
                ...
                if(in_array($this->capture_params['payment_type'], array('INVOICE_START', 'GUARANTEED_INVOICE'))) {
                    ...
                    $comments .= $this->model_extension_payment_novalnet->prepareNovalnetComments($this->capture_params, $order_info, $this->transaction_history['payment_type'], $this->transaction_history['order_no']);
                }
            ...
            if($this->transaction_history['gateway_status'] == 75 && in_array($this->capture_params['tid_status'], array(91, 99))) {
                ...
                $confirmed_comments = 'Novalnet callback received. The transaction has been confirmed  successfully for the TID: ' .$this->capture_params['shop_tid'].  ' on '. date('Y-m-d H:i:s');
            }
            ...
            $this->updateCallbackComments(array(
            'order_no' => $this->transaction_history['order_no'],
            'comments' => $confirmed_comments,
            'orders_status_id' => $order_status
            ));


            // Send notification mail.
            $this->sendNotificationMail($comments . $confirmed_comments);
            $this->displayMessage($comments . $confirmed_comments);           
    ...
    public function communicationFailureProcess($order_id)
    {
        if ($order_id) {
            ...
            if (!in_array($this->capture_params['payment_type'], $this->payment_group[$payment_type])) {
                ...
                $config_key = $this->model_extension_payment_novalnet->dbSelect('setting', 'value', 'code ="' . $payment_type . '" AND `key` =' . $payment_order_status);
    ...
    {
        $error = '';
        if (!empty($this->capture_params['vendor_activation'])) {
            foreach ($this->affiliate_params as $val) {
                if (empty($this->capture_params[$val])) {
                    $error .= 'Required param ( ' . $val . ' ) missing! <br />';
                }
            }
            if ($error != '') {
    ...
    /**
     * Update affliate account activation details in novalnet_affiliate_account table
     *
     * @param       $data
     * @return      none
     */
    public function updateAffiliateDetail($data = array())
    {
        if (!empty($data)) {
            $param['activation_date'] = $data['activation_date'];
            $this->model_extension_payment_novalnet->dbInsert('novalnet_affiliate_account', array(
                'aff_id',
                'affiliate_configuration',
                'product_url',
                'activation_date',
            ), array(
                $this->db->escape($data['aff_id']),
                "'" . json_encode(array(
                    'vendor_id'       => $this->db->escape($data['vendor_id']),
                    'vendor_authcode' => $this->db->escape($data['vendor_authcode']),
                    'product_id'      => $this->db->escape($data['product_id']),
                    'aff_authcode'    => $this->db->escape($data['aff_authcode']),
                    'aff_accesskey'   => $this->db->escape($data['aff_accesskey'])
                )) . "'",
                "'" . $this->db->escape($data['product_url']) . "'",
                "'" . $this->db->escape($param['activation_date']) . "'",
            ));
        }
    }
    ...
    public function transaction_cancellation(){
        ...
        if ( 'TRANSACTION_CANCELLATION' === $this->capture_params ['payment_type'] && in_array( $selected_details['gateway_status'], array('75','91','99','98','85', '90', '86' )) || (in_array( $this->capture_params['payment_type'], array('GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA' )) && $this->capture_params ['tid_status'] == '103' && in_array( $selected_details['gateway_status'], array('75','91','99','98','85', '90', '86' )))) {
            $novalnet_comments ='Novalnet callback received. The transaction has been canceled on ' .date('Y-m-d H:i:s') . '.';
            // Update order status ID in shop order table.
            $this->model_extension_payment_novalnet->dbUpdate('order', array(
