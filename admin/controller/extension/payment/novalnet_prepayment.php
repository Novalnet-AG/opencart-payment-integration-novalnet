...
class ControllerExtensionPaymentNovalnetPrepayment extends Controller
{
	...
	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$this->model_extension_payment_novalnet->storeConfigurations('payment_novalnet_prepayment');
		}

		// Get template details.
		$data = $this->model_extension_payment_novalnet->getTemplateDetails('novalnet_prepayment');
		...
		// Add template.
		$this->response->setOutput($this->load->view('extension/payment/novalnet_prepayment', $data));
	}
	...
	public function getConfigurationContent(&$data) {
		foreach (array(
			'payment_novalnet_prepayment_status',
			'payment_novalnet_prepayment_testmode',
			'payment_novalnet_prepayment_due_date',
			'payment_novalnet_prepayment_buyer_notification',
			'payment_novalnet_prepayment_order_completion_status',
			'payment_novalnet_prepayment_callback_order_status',
			'payment_novalnet_prepayment_total',
			'payment_novalnet_prepayment_geo_zone_id',
			'payment_novalnet_prepayment_sort_order',
		) as $field) {
	...
	public function getLanguageContent(&$data) {
		foreach (array(
			...
			'text_enabled',
			'text_disabled',
			'text_prepayment_duedate',
			'text_prepayment_duedate_desc',
			'text_novalnet_prepayment',
			'logo_novalnet_prepayment',
	...
	public function order() {
		if ($this->config->get('payment_novalnet_prepayment_status')) {
